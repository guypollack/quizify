...
export function SongSearchContainer({searchTerms, setSearchTerms, searchSuggestions, currentSongInput, setCurrentSongInput}) {
  ...
  return (
    <div className="song-search">
      {[1,2,3,4,5].map(num => {
        return <SongSearch showList={true} key={num.toString()} inputNumber={num.toString()} searchTerm={searchTerms[num]} searchSuggestions={searchSuggestions[num]} onFocus={handleFocus} onChange={handleChange} />
      })}
    </div>
  )
};